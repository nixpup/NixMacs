...
** Define Packages
If this is the first time this configuration is used, and the automatic package installation didn't work, run ~M-x package-install-selected-packages~.
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(color-theme-sanityinc-tomorrow company emms fancy-dabbrev lsp-mode
				    lsp-ui markdown-mode multi-term
				    multiple-cursors nix-buffer
				    nix-mode rainbow-mode rust-mode
				    rustic wttrin hydra all-the-icons haskell-mode arduino-mode flycheck gruvbox-theme bongo)))
#+END_SRC
** Automatically Install Packages
This function should ideally automatically install all required packages for you into your emacs setup.
#+BEGIN_SRC emacs-lisp
  (defun my/ensure-selected-packages-installed ()
  "Install missing packages from `package-selected-packages` without refreshing every time."
  (unless (cl-every #'package-installed-p package-selected-packages)
    (message "Refreshing package contents...")
    (package-refresh-contents)
    (package-install-selected-packages :noconfirm)))

  (my/ensure-selected-packages-installed)
#+END_SRC
** Colorscheme and Font Size
...
#+END_SRC
** Fancy Dabbrev (Simple Auto-Completion)
...
#+BEGIN_SRC emacs-lisp
  ...
  (httpd-start)
  (impatient-mode 1)
  ;; Open browser tab with the impatient-mode preview URL
...
#+END_SRC
*** Hoon Mode and Recognition
Set up Hoon syntax highlighting with hoon-mode and the included rune dictionary.
#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'package  
    (unless (package-installed-p 'hoon-mode)
      (package-vc-install "https://github.com/urbit/hoon-mode.el.git")))
...
#+BEGIN_SRC emacs-lisp
      (use-package bongo
        :ensure t
        :commands (bongo-playlist bongo-library)
        :config
      ...
      (let ((inhibit-read-only t))
        ...
        (let* ((window-width (window-width))
          ...
          (when (display-graphic-p)
            (let ((image (create-image "~/Pictures/nix_emacs_logo_small.png" nil nil :ascent 'center)))
              (when image
