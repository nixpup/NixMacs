...
#+END_SRC
Show the full path to the currently open file in the modeline.
#+BEGIN_SRC emacs-lisp
...
(when (and (eq window-system 'x) (featurep 'exwm))
  (load-file "~/.nixmacs/config/exwm.el"))
#+END_SRC
      ...
      (let ((inhibit-read-only t))
        ...
        (let* ((window-width (window-width))
          ...
          (when (display-graphic-p)
            (let ((image (create-image (or (getenv "NIXMACS_LOGO_PATH") "~/Pictures/nix_emacs_logo_small.png") nil nil :ascent 'center)))
              (when image
...
#+END_SRC
** Font Height Keybinds
Bind `C-x C-S-Minus` and `C-x C-S-=` to decrease and increase the font height.
#+BEGIN_SRC emacs-lisp
...
(defun my/toggle-line-numbers ()
  "Toggle global display of line numbers."
  (interactive)
  (if global-display-line-numbers-mode
      (global-display-line-numbers-mode -1)
    (global-display-line-numbers-mode 1)))

(global-set-key (kbd "C-x C-S-n") #'my/toggle-line-numbers)
#+END_SRC
** Org Insert Helper
